...
contract LpPool is Governance, IERC721Receiver {
    using SafeMath for uint256;
    ...
    bool private _isAward = true;
    uint private _round;
    // round -> nftTokenId
    mapping(uint => uint256) private _roundTokenId;
    // round -> winner
    mapping(uint => address) private _roundwinner;
    // Probability of users
    mapping(address => AccountProb) private _accountProb;
    mapping(address => GfvProfit) private _gfvProfit;
    // participant
    address[] _accounts;
    // Total pledge
    uint256 private _totalLp;
    // Current pledge per user
    ...
    function getGFVProfitNum(address account) public view returns (uint256) {

        return calcGFV(account, _lpBalances[account]);
    }
     ...
     *
     */
    function setNFTTokenId(uint256 nftTokenId) external onlyGovernance {
        require(_isAward, "Prize cannot be set repeatedly!");
...
//    }
     ...
     *
     */
    function pledgeLp(uint256 amount) external {
    ...
    function award() external onlyGovernance returns (address) {
        ...
        for(uint256 i = 0; i < _accounts.length; i++){
            addr = _accounts[i];
            if(_lpBalances[addr] > 0) {
                uint pledgeTime = currentTime.sub(_accountProb[addr].time);
                _accountProb[addr].prob = _accountProb[addr].prob.add(_lpBalances[addr].mul(pledgeTime));
                _accountProb[addr].time = currentTime;
        ...
        _roundwinner[_round] = awardAccount;
        _nft.safeTransferFrom(address(this), awardAccount, _roundTokenId[_round]);
        _isAward = true;
        return awardAccount;
    }
    ...
    function getRewards(uint round) external view returns (address, uint256) {
        ...
        return (_roundwinner[round], _roundTokenId[round]);
    }
    ...
    function currentIsAward() external view returns (bool) {
